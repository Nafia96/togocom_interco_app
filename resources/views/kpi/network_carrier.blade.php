...
<head>
<meta charset="UTF-8">
<title>Network Carrier KPI</title>
...
<div class="card-header bg-warning fw-bold d-flex justify-content-between align-items-center">
    <span>
        NETWORK × CARRIER KPI – {{ strtoupper($direction) }} – {{ strtoupper($view) }}
    </span>
...
<div class="card-body">
    ...
    <div class="d-flex gap-2 mb-3">
        <a href="?direction=ORIGINATED&view={{$view}}&start_date={{$start}}&end_date={{$end}}&network={{$network}}&partner={{$partner}}"
        ...
        <a href="?direction=DESTINATED&view={{$view}}&start_date={{$start}}&end_date={{$end}}&network={{$network}}&partner={{$partner}}"
...
<form method="GET" class="row g-2 mb-3">
    ...
    <div class="col-md-3">
        ...
        <select name="network" class="form-select" onchange="this.form.submit()">
            ...
            @foreach($networksList as $n)
                <option value="{{ $n }}" {{ $network==$n?'selected':'' }}>
                    {{ $n }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <select name="partner" class="form-select" onchange="this.form.submit()">
            ...
            @foreach($partnersList as $p)
                <option value="{{ $p }}" {{ $partner==$p?'selected':'' }}>
                    {{ $p }}
                </option>
            @endforeach
        </select>
...
<tbody>
@foreach($networks as $networkName => $partners)
    @foreach($partners as $partnerName => $rows)
        <tr>
