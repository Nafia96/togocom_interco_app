...
<body>
@php
    $direction = $direction ?? request()->get('direction', 'IN');
    $view = $view ?? request()->get('view', 'day');
    $start = $start ?? request()->get('start_date', now()->startOfMonth()->toDateString());
    $end = $end ?? request()->get('end_date', now()->endOfMonth()->toDateString());
@endphp
