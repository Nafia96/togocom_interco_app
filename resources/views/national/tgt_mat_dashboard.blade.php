        ...
        <ol class="breadcrumb">
            ...
            <div class="d-flex justify-content-end container-fluid mt-n3">
                @if (getUserType()->type_user == 3 || getUserType()->type_user == 2)
                    <a data-toggle="modal" data-target="#addMesurModal11" data-direction="TGT->MAT"> <button type="button" class=" btn btn-dark mx-1">+ AJOUTER
                            MESURE</button></a>
                @endif
            </div>
        </ol>
...
<script>
    $(function(){
        ...
        $(document).on('submit', '#setValidatedForm_TGT_MAT', function(e){
            e.preventDefault();
            const form = this;
            const value = $('#traffic_validated_input_TGT_MAT').val();
            const comment = $('#validation_comment_input_TGT_MAT').val();
