...
$query = "SELECT direction, network, carrier, sum(attempt) attempt, concat(round(sum(completed)/ sum(attempt) * 100), '%') ner, concat(round(sum(answered)/ sum(attempt) * 100),'%') asr, round((sum(duration_min)*60)/ sum(answered)) acd_sec FROM network_carrier";
$queryCount = "SELECT count(a.direction) count FROM (SELECT direction, network, carrier, sum(answered) FROM network_carrier";
$params = [];
$sortable = ["direction", "network", "carrier", "attempt", "ner", "asr", "acd_sec"];
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Stat Network Carrier Grouping </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_stat_network_carrier_group.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div>             
<br/>
