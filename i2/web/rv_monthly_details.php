...
$query = "SELECT substr(start_date,1,7) start_month, carrier, details, sum(duration) duration, tarif, round(sum(revenue_cfa)) revenue_cfa, round(sum(revenue_cfa)/655.957,3) revenue_euro FROM `revenu`";
$queryCount = "SELECT COUNT(a.start_month) as count FROM (SELECT substr(start_date,1,7) start_month, carrier, details, round(sum(revenue_cfa)) revenue_cfa FROM `revenu`";
$params = [];
...
$query .= " GROUP BY start_month, carrier, details, tarif";
$queryCount .= " GROUP BY start_month, carrier, details)a";
...
if(!empty($_GET['sort']) && in_array($_GET['sort'], $sortable)){
    ...
    $query .= " ORDER BY start_month desc, carrier, details, tarif";
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Monthly Revenue Details </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_monthly_details.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div> 
<br/>
		...
		<div class="tableau" >
		    <table class="table table-striped">
                        ...
                        <tr>
                                ...
                                <th><?= TableHelper::sort('tarif', 'tarif', $_GET) ?></th>
