...
$query = "SELECT direction, start_date, network, carrier, answered, duration_min FROM network_carrier";
$params = [];
$dirList = ["Incoming", "Outgoing"];

// dates controle
if((empty($_SESSION['q3']) && !empty($_SESSION['q4'])) || (!empty($_SESSION['q3']) && empty($_SESSION['q4']))){
    $date_error = "Paramètrage de dates invalide !";
}

// #0 Affichage sans parametre
if(!in_array($_SESSION['direction'], $dirList) && empty($_SESSION['q1']) && empty($_SESSION['q2']) && empty($_SESSION['q3']) && empty($_SESSION['q4'])){
...
array_push($list, array("direction, start_date, network, carrier, nb_call, volume_min"));
