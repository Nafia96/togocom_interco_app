...
$params = [];
$sortable = ["direction","start_date", "carrier", "trunk_id", "attempt","SIP_0","SIP_1","SIP_2","SIP_400","SIP_403","SIP_404","SIP_408","SIP_410","SIP_480","SIP_481","SIP_482","SIP_483","SIP_484","SIP_486","SIP_487","SIP_500","SIP_501","SIP_502","SIP_503","SIP_504","SIP_603","SIP_604","SIP_606"];
...
if(in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && !empty($_GET['q2']) && !empty($_GET['q3']) && !empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND start_date BETWEEN :begin_date AND :end_date AND trunk_id LIKE :trunk_id AND carrier LIKE :carrier";
...
if(!in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && empty($_GET['q2']) && empty($_GET['q3']) && empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE trunk_id LIKE :trunk_id AND start_date = CURRENT_DATE-1";
	$params['trunk_id'] = '%' . $_GET['q1'] . '%';
}
...
if(in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && empty($_GET['q2']) && empty($_GET['q3']) && empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND trunk_id LIKE :trunk_id AND start_date = CURRENT_DATE-1";
	$params['direction'] = '%' . $_GET['direction'] . '%';
    $params['trunk_id'] = '%' . $_GET['q1'] . '%';
}
...
if(!in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && empty($_GET['q2']) && !empty($_GET['q3']) && !empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE trunk_id LIKE :trunk_id AND start_date BETWEEN :begin_date AND :end_date";
    $params['trunk_id'] = '%' . $_GET['q1'] . '%';
    $params['begin_date'] = $_GET['q3'];
    $params['end_date'] = $_GET['q4']; 
}
...
if(!in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && !empty($_GET['q2']) && empty($_GET['q3']) && empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE trunk_id LIKE :trunk_id AND carrier LIKE :carrier AND start_date = CURRENT_DATE-1";
	$params['trunk_id'] = '%' . $_GET['q1'] . '%';
    $params['carrier'] = '%' . $_GET['q2'] . '%';
}
...
if(in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && empty($_GET['q2']) && !empty($_GET['q3']) && !empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND trunk_id LIKE :trunk_id AND start_date BETWEEN :begin_date AND :end_date";
	$params['direction'] = '%' . $_GET['direction'] . '%';
    $params['trunk_id'] = '%' . $_GET['q1'] . '%';
    $params['begin_date'] = $_GET['q3'];
    $params['end_date'] = $_GET['q4']; 
...
if(in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && !empty($_GET['q2']) && empty($_GET['q3']) && empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND trunk_id LIKE :trunk_id AND carrier LIKE :carrier AND start_date = CURRENT_DATE-1";
	$params['direction'] = '%' . $_GET['direction'] . '%';
    $params['trunk_id'] = '%' . $_GET['q1'] . '%';
    $params['carrier'] = '%' . $_GET['q2'] . '%';
}
...
if(!in_array($_GET['direction'], $direction_list) && !empty($_GET['q1']) && !empty($_GET['q2']) && !empty($_GET['q3']) && !empty($_GET['q4'])){
    ...
    $queryCount .= " WHERE trunk_id LIKE :trunk_id AND carrier LIKE :carrier AND start_date BETWEEN :begin_date AND :end_date";
    $params['trunk_id'] = '%' . $_GET['q1'] . '%';
    $params['carrier'] = '%' . $_GET['q2'] . '%';
    $params['begin_date'] = $_GET['q3'];
    $params['end_date'] = $_GET['q4']; 
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Carrier Trunk-ID SIP Error  </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_carrier_trunkid_sip_error.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div>
		    ...
		    <table class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th><?= TableHelper::sort('carrier', '_____________carrier_____________', $_GET) ?></th>
