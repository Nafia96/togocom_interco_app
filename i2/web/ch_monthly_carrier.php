...
$query = "select direction, substr(start_date,1,7) start_month, carrier, sum(nb_calls) nb_calls, round(sum(volume_min),3) volume_min, round(sum(charge_euro),3) charge_euro, round(sum(charge_cfa),3) charge_cfa from `charge`";
$queryCount = "SELECT COUNT(a.start_month) as count FROM (SELECT substr(start_date,1,7) start_month, carrier, round(sum(volume_min)) volume_min FROM `charge`";
$params = [];
$sortable = ["start_month", "carrier", "nb_calls", "volume_min", "charge_euro", "charge_cfa"];
...
$query .= " GROUP BY direction, start_month, carrier";
$queryCount .= " GROUP BY direction, start_month, carrier)a";
...
if(!empty($_GET['sort']) && in_array($_GET['sort'], $sortable)){
    ...
    $query .= " ORDER BY direction, start_month, carrier";
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Monthly Charge Carrier </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_ch_monthly_carrier.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div> 
<br/>
