...
//$network_set = $_GET['network'];
$query = "SELECT direction, network, sum(attempt) attempt, concat(round(sum(completed)/ sum(attempt) * 100), '%') ner, concat(round(sum(answered)/ sum(attempt) * 100),'%') asr, round((sum(duration_min)*60)/ sum(answered)) acd_sec FROM network_traffic";
$queryCount = "SELECT count(a.direction) count from (select direction, network, sum(attempt) FROM network_traffic";
$params = [];
$sortable = ["direction", "network", "attempt", "ner", "asr", "acd_sec"];
$direction_list = ["Incoming", "Outgoing"];
//$network_list = ["BTS", "BICS"];
//$querynetwork = "SELECT DISTINCT network_name FROM network_tab ORDER BY network_name";
//$querynetwork_set = "SELECT DISTINCT network_name FROM network_tab WHERE network_name=:network_name";
...
//$count2 = $bdd->prepare("SELECT DISTINCT network_name FROM network_tab WHERE network_name=:network_name");
//$count2->bindParam(":network_name", $network_set);
//$count2->execute();
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Stat Network Grouping </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_stat_network_group.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div>      
<br/>
