...
$query = "SELECT a.start_date start_date, a.duration duration, a.revenue_cfa revenue_cfa from (select start_date, sum(duration) duration, round(sum(revenue_cfa)) revenue_cfa FROM `revenu` group by start_date)a";
...
if(!empty($_GET['q1']) && !empty($_GET['q2'])) {
	$query .= " WHERE start_date BETWEEN :st_date AND :end_date";
    	$queryCount .= " WHERE a.start_date BETWEEN :st_date AND :end_date";
