...
$query = "SELECT * FROM hourly_network";
$queryCount = "SELECT COUNT(start_hour) as count FROM hourly_network";
$params = [];
$sortable = ["direction", "start_date", "start_hour", "network", "attempt", "ner", "asr", "acd_sec"];
...
if(in_array($_GET['direction'], $direction_list) && in_array($_GET['start_hour'], $hour_list) && !empty($_GET['q1']) && !empty($_GET['q2']) ){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND start_hour LIKE :start_hour AND start_date LIKE :start_date AND network LIKE :network";
...
if(!in_array($_GET['direction'], $direction_list) && in_array($_GET['start_hour'], $hour_list) && empty($_GET['q1']) && !empty($_GET['q2'])){
    ...
    $queryCount .= " WHERE start_hour LIKE :start_hour AND network LIKE :network AND start_date = CURRENT_DATE-1";
	$params['start_hour'] = '%' . $_GET['start_hour'] . '%';
    $params['network'] = '%' . $_GET['q2'] . '%';
}
...
if(in_array($_GET['direction'], $direction_list) && in_array($_GET['start_hour'], $hour_list) && empty($_GET['q1']) && !empty($_GET['q2'])){
    ...
    $queryCount .= " WHERE direction LIKE :direction AND start_hour LIKE :start_hour AND network LIKE :network AND start_date = CURRENT_DATE-1";
	$params['direction'] = '%' . $_GET['direction'] . '%';
    $params['start_hour'] = '%' . $_GET['start_hour'] . '%';
    $params['network'] = '%' . $_GET['q2'] . '%';
}
...
if(!in_array($_GET['direction'], $direction_list) && in_array($_GET['start_hour'], $hour_list) && !empty($_GET['q1']) && !empty($_GET['q2'])){
    ...
    $queryCount .= " WHERE start_hour LIKE :start_hour AND start_date LIKE :start_date AND network LIKE :network";
	$params['start_hour'] = '%' . $_GET['start_hour'] . '%';
    $params['start_date'] = '%' . $_GET['q1'] . '%';
    $params['network'] = '%' . $_GET['q2'] . '%';
}
...
if(!empty($_GET['sort']) && in_array($_GET['sort'], $sortable)){
    ...
    $query .= " ORDER BY start_date desc, start_hour desc, network";
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Hourly Network </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_hourly_network.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div>      
<br/>
