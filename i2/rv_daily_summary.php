...
$query = "SELECT start_date, sum(duration) duration, round(sum(revenue_cfa)) revenue_cfa, round(sum(revenue_cfa)/655.957) revenue_euro FROM `revenu`";
$queryCount = "SELECT COUNT(a.start_date) as count from (select start_date, sum(revenue_cfa) FROM `revenu` group by start_date)a";
$params = [];
$sortable = ["start_date", "duration", "revenue_euro", "revenue_cfa"];
...
?>
   <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Daily Revenue Summary </h2>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="float-right">
                <a href="export_daily_summary.php" class="btn btn-success"><i class="dwn"></i> Export to CSV</a>
            </div>
        </div>
    </div>                 
<br/>
