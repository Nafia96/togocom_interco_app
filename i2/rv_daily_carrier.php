...
$query = "SELECT start_date, carrier, sum(duration) duration_min, round(sum(revenue_cfa)) revenue_cfa, round(sum(revenue_cfa)/655.957) revenue_euro  FROM `revenu`";
$queryCount = "SELECT COUNT(a.start_date) count FROM (SELECT start_date, carrier, sum(revenue_cfa) FROM `revenu`";
$params = [];
$sortable = ["start_date", "carrier", "revenue_cfa", "revenue_euro"];
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h2 class="h2"> Daily Revenue Carrier </h2>
